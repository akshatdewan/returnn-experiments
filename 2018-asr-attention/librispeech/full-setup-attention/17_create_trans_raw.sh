...
for prefix in demo; do
    if test -s data/dataset_fr_cv_euparl-st_euparlpost2018_ilo_unog/$prefix.trans.raw; then
      echo "$prefix exists already"
    continue
    fi
    ./returnn/tools/dump-dataset-raw-strings.py --dataset "{'class':'SpecAugFrCorpus', 'bpe':{'bpe_file':'data/dataset_fr_cv_euparl-st_euparlpost2018_ilo_unog/trans.bpe.codes', 'vocab_file':'data/dataset_fr_cv_euparl-st_euparlpost2018_ilo_unog/trans.bpe.vocab','unknown_label':'<unk>'}, 'path':'data/dataset_fr_cv_euparl-st_euparlpost2018_ilo_unog/', 'audio':{}, 'prefix': '$prefix', 'use_zip': False}" --out data/dataset_fr_cv_euparl-st_euparlpost2018_ilo_unog/$prefix.trans.raw
done
